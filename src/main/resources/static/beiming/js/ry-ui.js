...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            exportExcel: function(formId) {
                $.modal.confirm("确定导出所有" + $.table._option.modalName + "吗？", function() {
                    ...
                    console.log("exportUrl:"+$.table._option.exportUrl);
        ...
        modal: {
            ...
            open_no_button: function (title, url, width, height, callback) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
            ...
            openFull_2: function (title, url, width, height) {
                //如果是移动端，就使用自适应大小弹窗
                if (navigator.userAgent.match(/(iPhone|iPod|Android|ios)/i)) {
                    width = 'auto';
                    height = 'auto';
        ...
        // 操作封装处理
        operate: {
            generalAjax:function(url,type,data,callBack){
                return $http({
                    url:url,
                    method:type,
                    data:data,
            ...
            edit_audit: function (title,id) {
                if ($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    ...
                    $.modal.open_no_button(title, url);
                } else {
                    $.modal.open_no_button(title, $.operate.audit(id));
                }
            },
            // 审核信息
            project_update: function (title,id) {
                if ($.common.isEmpty(id) && $.table._option.type == table_type.bootstrapTreeTable) {
                    var row = $('#' + $.table._option.id).bootstrapTreeTable('getSelections')[0];
                    if ($.common.isEmpty(row)) {
                        $.modal.alertWarning("请至少选择一条记录");
                    ...
                    var url = $.table._option.updateReportUrl.replace("{id}", row[$.table._option.uniqueId]);
                    $.modal.openTab("修改" , url);
                } else {
                    $.modal.openTab("修改", $.operate.updateReportUrl(id));
                }
            },
            // 修改信息，以tab页展现
            editTab: function (id) {
            ...
            editFull_2: function (id) {
                ...
                $.modal.openFull_2("修改" + $.table._option.modalName, url);
            },
            allotFull: function (id) {
                var url = "/404.html";
                if ($.common.isNotEmpty(id)) {
                    url = $.table._option.allotUrl + "/" + id;
                } else {
                    var row = $.common.isEmpty($.table._option.uniqueId) ? $.table.selectFirstColumns() : $.table.selectColumns($.table._option.uniqueId);
                    url = $.table._option.allotUrl + "/" + row;
                }
                $.modal.openFull_2("分配" + $.table._option.modalName, url);
            },
            // 修改访问地址
            editUrl: function (id) {
                var url = "/404.html";
            ...
            updateReportUrl: function (id) {
                ...
                if ($.common.isNotEmpty(id)) {
                    ...
                    url = $.table._option.updateReportUrl.replace("{id}", id);
                }
                return url;
            },
            // 审核访问地址
            audit: function (id) {
                ...
                if ($.common.isNotEmpty(id)) {
                    ...
                    url = $.table._option.auditURl.replace("{id}", id);
                }
                return url;
            },
            // 保存信息 刷新表格
            ...
            // 保存信息 弹出提示框
            saveModalPut: function (url, data) {
                var config = {
                    url: url,
                    type: "put",
                    dataType: "json",
            ...
            saveTab2: function (url, data) {
                var config = {
                    ...
                    success: function (result) {
                        $.operate.successTabCallback2(result);
                    }
                };
                $.ajax(config)
            },
            ...
            successTabCallback: function (result) {
                if (result.code == web_status.SUCCESS) {
                    ...
                    var $contentWindow = $('.beiming_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
                    $.modal.close();
            ...
            // 选项卡成功刷新
            successTabCallback2: function (result) {
                if (result.code == web_status.SUCCESS) {
                    var topWindow = $(window.parent.document);
                    var currentId = $('.page-tabs-content', topWindow).find('.active').attr('data-panel');
                    var $contentWindow = $('.beiming_iframe[data-id="' + currentId + '"]', topWindow)[0].contentWindow;
